...
def predict_class():
    ...
    genres_encoded = le1.transform([genres])[0]
    prod_comp_encoded = le2.transform([production_companies])[0]
    budget_scaled = scaler1.transform([[budget]])[0]
    runtime_scaled = scaler2.transform([[runtime]])[0]
    month = np.array([month])
    year = np.array([year])
    day_of_week = np.array([day_of_week])

    input_array = np.concatenate((genres_encoded, prod_comp_encoded, budget_scaled, runtime_scaled, month, year, day_of_week), axis=None)
